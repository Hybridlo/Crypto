...
class MongometryTest {
    ...
    void testMontgomeryMultiply() {
        ...
        Montgomery m = new Montgomery(lim);
        for (int i = 0; i < iter; i++) {
            ...
            assertEquals(a.multiply(b).mod(lim), m.convertOut(m.multiply(m.convertIn(a),m.convertIn(b))));
        }
    }
    ...
    void testMontgomeryPow() {
        ...
        for (int i = 0; i < iter; i++) {
            ...
            assertEquals(a.modPow(b, lim), m.convertOut(m.modPow(m.convertIn(a), b)));
        }
    }
}